...
export const STEP_CONFIG: StepConfig[] = [
  ...
  },
  {
    ...
    nextStepHint: "Danach geht es weiter.",
    stage: "detail",
    fieldIds: ["airOutdoorUnitSpace", "airAccessWidth", "airOutdoorToTechnicalRoomDistance", "airNoiseSensitivity", "heatingCoolingInterest"],
...
export const FIELD_CONFIG: FieldConfig[] = [
  ...
  defineField({
    ...
    stepId: "heating-existing-system",
    label: "Betriebsstunden der Anlage",
    kind: "choice-single",
    priority: "recommended",
    ...
    options: [
      ...
      { label: "1.000–2.000 Stunden/Jahr", value: "1000-2000" },
      { label: "Über 2.000 Stunden/Jahr", value: "ueber-2000" },
      { label: "Weiß ich nicht", value: "unbekannt" },
    ],
    visibleWhen: (values) =>
  ...
  defineField({
    id: "heatingDistrictHeat",
    stepId: "heating-existing-system",
    label: "Ist Fernwärme oder Nahwärme in deiner Straße verfügbar?",
    kind: "choice-single",
    priority: "required",
    ...
    visibleWhen: (values) => values.projectStandbein === "umruestung-heizung",
  }),
  defineField({
    id: "heatingCompetition",
    stepId: "heating-existing-system",
    label: "Hast du schon bei anderen Firmen angefragt?",
    kind: "choice-single",
    priority: "recommended",
    purpose: "Hilft uns, den nächsten Schritt richtig einzuschätzen.",
    outputKey: "heating.competition",
    options: [
      { label: "Nein, ihr seid die Ersten", value: "erst" },
      { label: "Ich habe schon ein Angebot", value: "ein-angebot" },
      { label: "Ich hole mehrere Angebote ein", value: "mehrere" },
    ],
    visibleWhen: (values) =>
  ...
  defineField({
    ...
    stepId: "heating-existing-system",
    label: "Welchen Kostenrahmen strebst du an?",
    kind: "choice-single",
    priority: "recommended",
    purpose: "Hilft uns, die Lösung von Anfang an auf dein Budget abzustimmen.",
    outputKey: "heating.budgetSegment",
    options: [
      { label: "Günstigste Lösung (ca. 15.000–20.000 €)", value: "budget" },
      { label: "Gutes Preis-Leistungs-Verhältnis (ca. 20.000–27.000 €)", value: "mittel" },
      { label: "Premium-Lösung (ab 27.000 €)", value: "premium" },
      { label: "Noch offen", value: "offen" },
    ],
    visibleWhen: (values) =>
      ["waermepumpen-austausch", "direktverdampfer-austausch", "umruestung-heizung"].includes(
  ...
  defineField({
    id: "heatingCoolingInterest",
    stepId: "heating-source-air",
    label: "Soll die Wärmepumpe auch zum Kühlen genutzt werden?",
    kind: "choice-single",
    priority: "recommended",
    ...
    options: [
      ...
      { label: "Nein, kein Thema", value: "nein" },
      { label: "Weiß ich nicht", value: "unbekannt" },
    ],
    visibleWhen: (values) =>
      values.heatingDistribution === "fussbodenheizung" || values.heatingDistribution === "gemischt",
  }),
  defineField({
  ...
  defineField({
    ...
    options: [
      { label: "Gut", value: "gut" },
      { label: "Eher eingeschränkt", value: "eingeschränkt" },
      { label: "Noch offen", value: "offen" },
    ],
  }),
  defineField({
  ...
  defineField({
    id: "pvGoal",
    stepId: "pv-new-base",
    label: "Was ist dein Hauptziel mit der PV-Anlage?",
    kind: "choice-single",
    priority: "required",
    purpose: "Ziel bestimmt Dimensionierung und Schwerpunkt der Planung.",
    outputKey: "pv.goal",
    required: true,
    options: [
      { label: "Möglichst viel Fläche nutzen (maximale Leistung)", value: "vollbelegung" },
      { label: "Eigenverbrauch optimal decken", value: "eigenverbrauch" },
      { label: "Bin offen – ich möchte eine Empfehlung", value: "empfehlung" },
    ],
    visibleWhen: (values) =>
  ...
  defineField({
    ...
    options: [
      ...
      { label: "Leerverrohrung vorhanden", value: "leerrohrung" },
      { label: "Über die Außenfassade", value: "fassade" },
      { label: "Weiß ich nicht", value: "unbekannt" },
    ],
    visibleWhen: (values) =>
      values.projectStandbein === "pv-neuanlage" || values.projectStandbein === "pv-erweiterung",
  ...
  defineField({
    id: "pvMainMeterPhoto",
    stepId: "pv-new-base",
    label: "Foto des Hauptverteilers (Zählerschrank)",
    kind: "file",
    priority: "required",
    purpose: "Unverzichtbar für die Angebotserstellung.",
    outputKey: "pv.photos.mainMeter",
    required: true,
    helperText: "Das Foto ist wichtig für die Angebotserstellung. Bitte Tür öffnen und gesamten Verteilerkasten abfotografieren.",
    helperCtaLabel: "Warum ist das wichtig?",
    helperTitle: "Warum das Foto nötig ist",
    helperBody: "Der Hauptverteiler zeigt uns die bestehende Absicherung, verfügbare Phasen und ob Platz für neue Zuleitungen ist — das brauchen wir für ein verbindliches Angebot.",
    visibleWhen: (values) =>
      values.projectStandbein === "pv-neuanlage" || values.projectStandbein === "pv-erweiterung",
  }),
  defineField({
    ...
    label: "Wie ist die Zählersituation?",
    kind: "choice-single",
    priority: "recommended",
    ...
    options: [
      ...
      { label: "Separate Zähler (z. B. WP getrennt)", value: "separat" },
      { label: "Weiß ich nicht", value: "unbekannt" },
    ],
    visibleWhen: (values) =>
      values.projectStandbein === "pv-neuanlage" || values.projectStandbein === "pv-erweiterung",
  ...
  defineField({
    ...
    options: [
      ...
      { label: "Eigenverbrauch erhöhen", value: "eigenverbrauch-erhöhen", visibleWhen: (values) => isPvRelatedStandbein(values) },
